...
export class MaterialPersianDateAdapter extends DateAdapter<jalaliMoment.Moment> {
  ...
  getMonthNames(style: "long" | "short" | "narrow"): string[] {
    switch (style) {
      case "long":
      case "short":
        return jalaliMoment.localeData("fa-IR").jMonths().slice(0);
      case "narrow":
        return jalaliMoment.localeData("fa-IR").jMonthsShort().slice(0);
    }
  }
  ...
  getDayOfWeekNames(style: "long" | "short" | "narrow"): string[] {
    switch (style) {
      case "long":
        return jalaliMoment.localeData("fa-IR").weekdays().slice(0);
      case "short":
        return jalaliMoment.localeData("fa-IR").weekdaysShort().slice(0);
      case "narrow":
  ...
  getFirstDayOfWeek(): number {
    return jalaliMoment.localeData("fa-IR").firstDayOfWeek();
  }
  ...
  createDate(year: number, month: number, date: number): jalaliMoment.Moment {
    ...
    const result = jalaliMoment()
      ...
      .hours(0).minutes(0).seconds(0).milliseconds(0).locale("fa-IR");
  ...
  today(): jalaliMoment.Moment {
    return jalaliMoment().locale("fa-IR");
  }

  parse(value: any, parseFormat: string | string[]): jalaliMoment.Moment | null {
    if (value && typeof value === "string") {
      return jalaliMoment(value, parseFormat, "fa-IR");
    }
    return value ? jalaliMoment(value).locale("fa-IR") : null;
  }
  ...
  override deserialize(value: any): jalaliMoment.Moment | null {
    ...
    if (typeof value === "string") {
      ...
      date = jalaliMoment(value).locale("fa-IR");
    }
